const API_KEY = '0aa41e38db067f59770abba18bdd8430';
...
const Tmdb = {
    ...
    getSpecificInfo: async (id, type) => {
        let info = {};

        if (id) {
            switch (type) {
                case 'movie':
                    info = await basicFetch(`/movie/${id}?language=es-ES&api_key=${API_KEY}`);
                    break;
                case 'tv':
